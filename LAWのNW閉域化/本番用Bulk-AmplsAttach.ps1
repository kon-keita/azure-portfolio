...
<# 
  AMPLS に LAW/AI をタグ条件（機能名称：3値 OR）で一括紐付け
  - ParentResource 非対応環境でも動作（REST 直叩き）
  - 既存スコープドリソースと差分のみ作成
  - ARM ベース URL は環境から自動検出（AzureCloud/China/USGov 等）
  - 認証トークンは CLI → Az.Accounts の順でフォールバック、JWT 形式チェックと期限切れ時の自動リトライを実装
  - タグ条件：機能名称 ∈ {製造現場アシスタントAI, セキュリティアドバイザー, 営業向けアシスタントAI}
#>
...
# --- タグ条件 ---(Tagkey,Tagvaluesに任意の値を設定)
$TagKey_Function    = 'AAAAAAA'
$TagValues_Function = @(
  'aaa',
  'bbb',
  'ccc'
...
$script:ArmBase            = $env.ResourceManagerUrl.TrimEnd('/')
$script:ArmResource        = "$($script:ArmBase)/"
$script:ArmResourceNoSlash = $script:ArmBase
...
# =========================================================
# 2) LAW/AI を取得 → タグ（機能名称のみで抽出）
# =========================================================
Write-Host "==== [2] LAW/AI を取得 → タグ（機能名称のみで抽出） ====" -ForegroundColor Cyan
$allTargets = @()
...
$filtered = $allTargets | Where-Object {
    ...
    # 機能名称は OR 条件（3値のいずれか）
    $hasFunc = $false
    if ($tags.ContainsKey($TagKey_Function)) {
        $val = [string]$tags[$TagKey_Function]
        if ($TagValues_Function -contains $val) {
            $hasFunc = $true
        }
    }
...
if (-not $filtered) {
  Write-Host "タグ条件（機能名称 ∈ {$($TagValues_Function -join ', ')}）に一致する LAW/AI がありません。" -ForegroundColor Yellow
  return
}
